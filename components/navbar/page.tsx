...
//           bg-white/10 backdrop-blur-xl border-b border-white/20 text-white shadow-md"
//         >
//           {/* Logo */}
//           <Link
//             href="/"
...
//                 hover:bg-white/10 hover:scale-105 transition transform"
//               >
//                 Sign In
//               </Link>
//               <Link
...
//           bg-white/10 backdrop-blur-xl border-t border-white/20 transition-all duration-300 
...
//       {/* Spacer (extra margin after navbar) */}
...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-3 w-full px-8">
            {!user ? (
              ...
              <Link
                href="/profile"
                onClick={() => setIsOpen(false)}
                className="w-full text-center px-5 py-2 rounded-lg font-semibold border border-white/30 flex items-center justify-center gap-2 hover:bg-white/10 transition"
              >
                {user.img_url ? (
                  <img
                    src={user.img_url}
      ...
      {/* Spacer (pushes content down so navbar doesn’t overlap) */}
      <div className="h-24 md:h-28"></div>
    </>
  );
}
