...
// // do the same for here but add more information in the view deatils page like mdoe what note stuff like that requested skill and rember only remove offered skill if mode is learno o if mode is exchnageing leave offered skill there ooo and dropf full code
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50 px-4">
          <div className="relative w-full max-w-md bg-gradient-to-br from-[#0b1228] to-[#020617] border border-white/20 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="space-y-3 text-gray-300">
              <p>
                <span className="text-gray-400">Requested:</span>{" "}
                {selectedReq.requested_skill_title}
              </p>
             ...
             {selectedReq.mode === "exchange" && selectedReq.skill_offered_title && (
              ...
              <p>
                <span className="text-gray-400">Mode:</span> {selectedReq.mode}
              </p>

              {selectedReq.note && (
                <p className="bg-white/5 p-3 rounded-xl border border-white/10">
                  <span className="text-gray-400">Note:</span> {selectedReq.note}
                </p>
              )}

              <p>
                <span className="text-gray-400">Status:</span> {selectedReq.status}
              </p>
              ...
              {selectedReq.status === "accepted" && (
                <Link
                  href={`/chat/${selectedReq.exchange_id}`}
                  className="block mt-6 text-center py-3 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 hover:opacity-90 transition font-semibold"
                >
                  Continue Chat 💬
                </Link>
              )}
